...
const STYLES: Record<OAuthProvider, { bg: string; color?: string; border?: string }> = {
...
interface Props {
  ...
  label: string
  loading: string | null
  onClick: (p: OAuthProvider) => void
}

export default function OAuthButton({ provider, label, loading, onClick }: Props) {
  const busy = loading === provider
  const { bg, color = '#fff', border } = STYLES[provider]
  return (
    <button
    ...
    >
      {busy ? <Loader2 style={{ width: 16, height: 16, animation: 'spin 1s linear infinite' }} /> : ICONS[provider]}
      <span>{label}</span>
    </button>
  )
}
