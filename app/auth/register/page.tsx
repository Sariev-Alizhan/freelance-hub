  ...
  return (
    <button
      ...
      style={{
        ...
        fontSize: full ? '14px' : '13px', fontWeight: 510, letterSpacing: '-0.01em',
      }}
    >
  ...
  return (
    <div className="min-h-[calc(100vh-52px)] flex items-center justify-center px-4 py-12" style={{ background: 'var(--fh-canvas)' }}>
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-xl p-5 space-y-4" style={{ background: 'var(--fh-surface)', border: '1px solid var(--fh-border-2)' }}>

          {/* Role selector */}
          <div>
            <p style={{ fontSize: '11px', fontWeight: 600, color: 'var(--fh-t3)', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
              I am registering as
            </p>
            <div className="grid grid-cols-2 gap-2">
              {([
                { value: 'client' as UserRole, icon: Briefcase, label: 'Client', sub: 'I post jobs' },
                { value: 'freelancer' as UserRole, icon: Code2, label: 'Freelancer', sub: 'I do work' },
              ]).map(({ value, icon: Icon, label, sub }) => {
                const active = role === value
                return (
                  <button key={value} onClick={() => setRole(value)}
                    className="flex flex-col items-center gap-1.5 p-3.5 rounded-lg transition-all"
                    style={{
                      border: active ? '1px solid rgba(113,112,255,0.45)' : '1px solid var(--fh-border)',
                      background: active ? 'rgba(113,112,255,0.09)' : 'var(--fh-surface-2)',
                    }}>
                    <Icon className="h-4 w-4" style={{ color: active ? '#7170ff' : 'var(--fh-t4)' }} />
                    <span style={{ fontSize: '13px', fontWeight: 590, color: active ? 'var(--fh-t1)' : 'var(--fh-t3)' }}>{label}</span>
          ...
          <div className="flex items-center gap-3">
            ...
            <span style={{ fontSize: '11px', color: 'var(--fh-t4)', fontWeight: 500, letterSpacing: '0.02em' }}>CONTINUE WITH</span>
            <div className="flex-1 h-px" style={{ background: 'var(--fh-sep)' }} />
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <OAuthButton provider="github"  label="GitHub"  icon={<GitHubIcon />}  bg="#24292e" loading={loading} onClick={signUp} />
              <OAuthButton provider="apple"   label="Apple"   icon={<AppleIcon />}   bg="#000000" loading={loading} onClick={signUp} />
            </div>

            <div className="grid grid-cols-2 gap-2">
              <OAuthButton provider="discord" label="Discord" icon={<DiscordIcon />} bg="#5865F2" loading={loading} onClick={signUp} />
              <OAuthButton provider="twitter" label="Twitter" icon={<XIcon />}       bg="#000000" loading={loading} onClick={signUp} />
            </div>
          </div>

          <p className="text-center" style={{ fontSize: '12px', color: 'var(--fh-t3)' }}>
            Commission: <span style={{ color: '#27a644', fontWeight: 590 }}>0%</span> forever
          </p>
        </div>
